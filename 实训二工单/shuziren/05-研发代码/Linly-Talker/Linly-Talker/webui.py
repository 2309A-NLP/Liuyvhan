...
# 从zhconv库导入convert函数，用于中文简繁转换。
from zhconv import convert
from LLM import LLM
# 从自定义的TTS模块导入EdgeTTS类，用于微软Edge的文本转语音服务。
...
# Default parameters
IMAGE_SIZE = 256
# PREPROCESS_TYPE：预处理方式为裁剪(crop)
PREPROCESS_TYPE = 'crop'
# FACERENDER：面部渲染器使用facevid2vid
...
# EXP_WEIGHT：表情权重为1
EXP_WEIGHT = 1
# USE_REF_VIDEO：不使用参考视频
USE_REF_VIDEO = False
# REF_VIDEO：参考视频为None
REF_VIDEO = None
# REF_INFO：参考信息为'pose'（姿势）
REF_INFO = 'pose'
# USE_IDLE_MODE：不使用空闲模式
USE_IDLE_MODE = False
# AUDIO_LENGTH：音频长度为5秒
AUDIO_LENGTH = 5
...
@calculate_time
def Asr(audio):
    # 定义语音识别函数，接收音频文件路径作为参数。
    try:
        # 调用全局asr对象的transcribe方法，将音频转换为文字。
...
def clear_memory():
    ...
    """
    # 1. 清理缓存的变量
    gc.collect()  # 触发Python垃圾回收 触发Python的垃圾回收器，回收不再使用的对象。
    torch.cuda.empty_cache()  # 清理PyTorch的显存缓存  清空PyTorch的CUDA缓存，释放显存。
    torch.cuda.ipc_collect()  # 清理PyTorch的跨进程通信缓存  清理PyTorch的跨进程通信（IPC）缓存。
    
    # 2. 打印显存使用情况（可选）
    # 打印当前显存使用情况（已分配、最大分配、缓存、最大缓存），单位是MB。
...
def generate_seed():
    # 生成一个1到1亿之间的随机整数，并以Gradio更新字典的形式返回，用于更新UI中的种子数值。
    seed = random.randint(1, 100000000)
    return {"__type__": "update", "value": seed}

def set_all_random_seed(seed):
    # 设置所有随机数生成器的种子（Python的random、NumPy、PyTorch CPU和GPU），确保结果可复现。
    random.seed(seed)
    np.random.seed(seed)
...
def change_instruction(mode):
    # 根据选择的TTS模式返回对应的操作说明文本。
    return instruct_dict.get(mode, '未知模式')

# 定义提示音频采样率为16000Hz，目标音频采样率为22050Hz。
PROMPT_SR, TARGET_SR = 16000, 22050
# 创建一个长度为22050的全零数组，作为默认音频数据（静音）。
DEFAULT_DATA = np.zeros(TARGET_SR)
'''
采样率是指每秒采集的音频样本数量，单位是Hz（赫兹）。
16000Hz (16kHz)：这是输入音频（提示音频）的采样率要求
大多数语音识别和语音克隆模型要求输入音频为16kHz
例如：用户上传的参考音频、麦克风录制的音频，都需要是16kHz
22050Hz (22.05kHz)：这是输出音频（合成语音）的采样率
合成的TTS语音输出为22.05kHz
比16kHz更高，音质更好（更清晰）
在代码中：DEFAULT_DATA = np.zeros(TARGET_SR) 创建了22050个采样点的静音数组
为什么不一样？
不同模型对音频格式的要求不同
输入用16kHz是为了兼容性和速度（处理更少的采样点更快）
输出用22.05kHz是为了音质（听起来更自然）
程序会在内部进行采样率转换（resampling）
'''

# TTS语音合成核心函数
'''
这三种TTS方法代表了三代不同的技术路线：
1.Edge-TTS（在线API）
特点：调用微软Azure的云端服务
优点：音质最好、支持多语言、情感丰富、无需本地GPU
缺点：必须联网、有延迟、可能有调用限制
实现：发送HTTP请求到微软服务器，返回音频数据
2.PaddleTTS（传统统计参数合成）
特点：基于声学模型+声码器的传统TTS
优点：完全离线、速度快、占用资源少
缺点：音质一般、机械感强、不自然
实现：本地加载预训练模型，CPU就能运行
3.GPT-SoVITS（深度学习语音克隆）
特点：基于Transformer的端到端语音克隆
优点：可以克隆任何人声音、情感自然、个性化强
缺点：需要高质量参考音频（3-10秒）、GPU要求高
实现：加载GPT和SoVITS两个大型神经网络模型
本质区别：技术路线完全不同，从"远程API"→"传统统计模型"→"深度学习端到端"，代表了TTS技术发展的三个阶段。
'''
@calculate_time
def TTS_response(text, voice, rate, volume, pitch, am, voc, lang, male,
                ...
                mode_checkbox_group, sft_dropdown, prompt_text_cv,
                 prompt_wav_upload, prompt_wav_record, seed, speed_factor,
                tts_method='Edge-TTS', save_path='answer.wav'):
    if text == '':
        text = '请输入文字/问题'

    if tts_method == 'Edge-TTS':
        # 如果是Edge-TTS方法，先检查网络连接，如果网络不通则警告并返回None。
        # edgetts 全局实例化对象
    ...
    if tts_method == 'GPT-SoVITS克隆声音':
        try:
            ...
            return None
    # CosyVoice系列方法（包括"SFT模式"和"克隆翻译模式"）
    # 优先使用上传的prompt音频，其次使用录制的，都没有则为None。
        ...
        if mode_checkbox_group in ['跨语种复刻']:
            # 如果是跨语种复刻模式，必须提供prompt音频，否则警告并返回静音数据
            if prompt_wav is None:
                gr.Warning('您正在使用跨语种复刻模式, 请提供prompt音频')
                '''
                Gradio的Audio组件期望的返回值有两种格式：
                文件路径：如 'answer.wav'
                元组 (采样率, numpy数组)：如 (22050, audio_data)
                这里返回的是静音音频：
                TARGET_SR = 22050：采样率
                DEFAULT_DATA = np.zeros(22050)：22050个静音采样点（即1秒的静音）
                为什么要返回静音？
                保持Gradio界面不崩溃（必须有音频输出）
                用静音代替错误提示，让用户知道音频生成失败
                这是一种优雅降级的处理方式
                '''
        ...
        # 对于"3s极速复刻"和"跨语种复刻"模式，检查prompt音频是否存在，以及采样率是否不低于16kHz。
        ...
        '''
        这三个分支对应CosyVoice的三种推理模式：
        1. 预训练音色 (SFT模式)
        输入：预定义音色ID（如"中文女"）
        原理：使用微调好的说话人嵌入
        应用：快速生成高质量、稳定的语音
        特点：不需要参考音频
        2. 3s极速复刻 (Zero-shot模式)
        输入：3秒参考音频 + 对应的文本
        原理：用编码器提取说话人特征，零样本迁移
        应用：克隆新声音，无需训练
        特点：需要prompt音频和文本
        3. 跨语种复刻 (Cross-lingual模式)
        输入：任意语言的参考音频
        原理：提取音色特征，忽略语言内容
        应用：让AI用中文说外语的声音
        特点：不需要prompt文本
        设计依据：这三种模式对应了CosyVoice的三大核心能力，每个模式的输入要求和处理逻辑都不同，所以需要分开处理。
        '''
        # 预训练音色模式：设置随机种子，调用CosyVoice的SFT预测方法。
...
def LLM_response(
    ...
    # 生成回答
    # 调用全局llm对象的generate方法，使用默认系统提示生成回答，并打印输出。
    answer = llm.generate(question, DEFAULT_SYSTEM)
    print("LLM 回复：", answer)

    # 合成回答语音
    # 将LLM生成的回答文本传给TTS函数，合成语音。
    ...
    # 如果是Edge-TTS则返回字幕文件路径（但下一行又强制设为None，所以实际不返回字幕），最后返回音频路径、字幕路径（None）和回答文本。
    tts_vtt = 'answer.vtt' if tts_method == 'Edge-TTS' else None
    # tts_vtt = None  如果想禁用字幕功能
...
def Talker_response_img(question_audio, method, text, voice, rate, volume, pitch,
    ...
    '''
    在视频生成中的含义：
    输入：生成的数字人视频每秒包含多少张图片
    值越大：视频越流畅（动作更连续），但生成时间更长，文件更大
    值越小：视频可能有卡顿感，但生成快
    '''
    # 如果启用了面部增强器但可能未安装，给出提示。
    if enhancer:
    ...
    if not voice:
        gr.Warning("请选择声音")
        return None
    # 调用LLM_response生成回答音频和文本（忽略回答文本）。
    ...
    if method == 'SadTalker':
        ...
        # 如果是Wav2Lip方法，调用其predict方法。
        video = talker.predict(source_image, driven_audio, batch_size)
    elif method == 'Wav2Lipv2':
        # 如果是Wav2Lipv2方法，调用其run方法。
        video = talker.run(source_image, driven_audio, batch_size)
    elif method == 'NeRFTalk':
        # 如果是NeRFTalk方法，调用其predict方法。
        video = talker.predict(driven_audio)
    else:
        gr.Warning("不支持的方法：" + method)
        return None
    # 如果有字幕文件则返回视频和字幕，否则只返回视频。
    return (video, driven_vtt) if driven_vtt else video

# 多轮对话函数
def chat_response(system, message, history):
    # response = llm.generate(message)
    # 调用LLM的chat方法，传入系统提示、当前消息和对话历史，返回回答和更新后的历史。
    response, history = llm.chat(system, message, history)
    # 打印对话历史（调试用）。
    print(history)
    # 流式输出
    # 逐字流式输出回答：每次增加一个字符，更新聊天界面。yield让函数成为生成器，实现流式效果。
    for i in range(len(response)):
        time.sleep(0.01)
        yield "", history[:-1] + [(message, response[:i+1])]
        '''
        返回值的结构：
        yield返回一个元组 (新消息, 新的对话历史)
        ""（第一个元素）
        清空输入框（表示消息已发送）
        对应UI中的msg输入框
        history[:-1] + [(message, response[:i+1])]（第二个元素）
        history[:-1]：去掉历史中最后一条（因为最后一条是用户消息，需要替换成带AI回答的完整对）
        [(message, response[:i+1])]：创建新的对话对
        message：用户的问题（完整）
        response[:i+1]：AI回答的前i+1个字符（逐渐增长）
        '''
    return "", history

# 修改系统提示词并清除对话历史。如果系统提示为空则使用默认值。返回三个值：系统提示、系统提示（用于UI）、空列表（清空聊天记录）。
...
def human_response(source_image, history, question_audio, talker_method, voice, rate, volume, pitch,
    ...
    if character == '女性角色':
        '''
        为什么要四个变量指向同一个值？
        source_image：原始图片路径，用于UI显示和模型输入
        pic_path：图片路径的别名
        crop_pic_path：裁剪后的图片路径，Wav2Lip需要裁剪
        first_coeff_path：SadTalker需要的第一帧系数
        '''
        source_image = pic_path = crop_pic_path = first_coeff_path = r'./inputs/girl.png'
        '''
        crop_info是预定义的面部裁剪信息：
        (403, 403)：裁剪后的大小（宽403，高403）
        (19, 30, 502, 513)：裁剪区域坐标（左上角x,y，右下角x,y）
        [40.06, 40.17, 443.79, 443.90]：面部关键点坐标
        为什么需要预裁剪？
        省去面部检测步骤，提高速度
        保证每次裁剪位置一致，结果稳定
        避免检测失败，提高成功率
        ...
        if source_image is None:
            ...
            return None
        default_voice = 'zh-CN-XiaoxiaoNeural' # ← 默认女性声音
    else:
        gr.Error("未知角色")
        return None
    ...
    if talker_method == 'SadTalker':
        # pose_style 随机生成的原因：多轮对话设计的拟人化效果
...
def MuseTalker_response(source_video, bbox_shift, question_audio, text, voice,
    ...
    # MuseTalker 视频生成
    # 调用musetalker的推理方法生成视频，帧率为25fps
...
# 加载VITS模型的函数：拼接完整路径，调用vits.load_model，返回路径。
...
def webui_setting(talk=False):
    # 如果talk为False（非对话模式），创建"数字人形象设定"标签页，包含图片上传组件；否则设为None。
    if not talk:
        with gr.Tabs():
    ...
    with gr.Tabs("TTS Method"):
        # 创建TTS方法的选项卡和可展开的折叠面板。
        with gr.Accordion("TTS Method语音方法调节 ", open=True):
            # Edge-TTS标签页：声音下拉菜单、速率、音量、音调滑块。
            ...
            # PaddleTTS标签页：声学模型、声码器、语言、性别复选框。
            ...
            with gr.Tab('GPT-SoVITS'):
                # GPT-SoVITS标签页：GPT模型和SoVITS模型的文件选择器，以及加载按钮。按钮点击时调用load_vits_model。
                with gr.Row():
                ...
                # 参考音频上传、是否使用麦克风、提示文本输入、语种选择，以及ASR按钮（自动识别参考音频的文字）。
                with gr.Row():
            ...
            with gr.Tab('CosyVoice'):
                ...
                # 创建prompt音频上传组件（只支持文件上传）
                # 创建prompt音频录制组件（只支持麦克风录制）。
                with gr.Row():
                    prompt_wav_upload = gr.Audio(sources='upload', type='filepath', label='选择prompt音频文件，注意采样率不低于16khz')
                    prompt_wav_record = gr.Audio(sources='microphone', type='filepath', label='录制prompt音频文件')
                # 创建prompt文本输入框，用于CosyVoice的零样本克隆。
...
def exmaple_setting(asr, text, character, talk, tts, voice, llm):
    # 默认text的Example
    examples = [
        # 定义第一个示例：使用Whisper-base识别，问压力管理，女性角色，SadTalker，Edge-TTS，晓晓声音，直接回复。
...
if __name__ == "__main__":
    ...
    try:
        '''
        asr是全局变量，在整个程序中都可以访问
        默认使用Whisper的"base"模型（轻量级版本）
        用户也可以通过UI切换模型，asr_model_change函数会重新赋值asr对象
        这是一种单例模式的设计，确保整个应用只加载一个ASR模型实例，节省内存
        '''
