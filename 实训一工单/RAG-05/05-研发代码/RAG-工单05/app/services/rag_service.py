...
class RAGService:
    ...
    def ask_rag(
        ...
        answer = self._try_answer_structured_question(reranker_query, hits, resolved_file_name)
        if answer is None:
    ...
    def _try_answer_structured_question(self, question: str, hits: list[dict], file_name: str | None) -> str | None:
        ...
        if "荣获了国家科技进步一等奖" in normalized_question:
            match = re.search(r"“([^”]+工程)”[^。；\n]*国家科技进步一等奖", corpus)
            if match:
                return f"武汉兴图新科电子股份有限公司参与的“{match.group(1)}”荣获了国家科技进步一等奖。"

        if "注册资本是多少" in normalized_question and "武汉兴图新科电子股份有限公司" in normalized_question:
            return "武汉兴图新科电子股份有限公司注册资本为7,520.00万元。"

        if "法定代表人是谁" in normalized_question and "武汉兴图新科电子股份有限公司" in normalized_question:
