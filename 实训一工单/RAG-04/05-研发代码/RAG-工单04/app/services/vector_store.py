...
class MilvusVectorStore:
    ...
    def upsert_chunks(self, doc_id: str, chunks: list[dict], embeddings: list[list[float]]) -> None:
        if not self.available or self.client is None:
            return
        if embeddings:
            self._sync_embedding_dimension(len(embeddings[0]))
    ...
    def search(self, query_vector: list[float], file_name: str | None, top_k: int) -> list[dict]:
        if not self.available or self.client is None:
            return []
        if query_vector:
            self._sync_embedding_dimension(len(query_vector))
        ...
        results = self.client.search(
            ...
            limit=top_k,
            output_fields=["chunk_id", "doc_id", "file_name", "page", "chunk_index", "block_type", "image_path", "content"],
            ...
            flattened.append(
                {
                    ...
                    "block_type": entity.get("block_type", "page_text"),
    ...
    def _sync_embedding_dimension(self, actual_dimension: int) -> None:
        if self.client is None or actual_dimension <= 0:
            return
